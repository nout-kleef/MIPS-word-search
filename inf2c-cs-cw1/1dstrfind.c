...
int main(void)
{
  ...
  do
  {
    c_input = dictionary[idx]; // dictionary stores the dictonary in a char[]
    if (c_input == '\0')
    {
      break;
    }
    if (c_input == '\n')
    {
      ...
      start_idx = idx + 1; // + 1 because it starts just after \n
    }
    idx += 1; // just keep incementing until we run into something interesting
  } while (1);
